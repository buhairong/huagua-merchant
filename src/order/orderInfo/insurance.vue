<template>
	<view class="page">
		<view>
			<view class="card" v-if="insuranceType1">
			...
			<view class="card" v-if="insuranceType2">
				<view class="title-wrap">
					<view class="title">
						机动车商业险
					</view>
...
export default {
	...
	onLoad(options) {
		...
		const insuranceList = uni.getStorageSync('insurance_list')
		if (insuranceList) {
			const insuranceType1 = insuranceList.find(item => item.insuranceType == 0)
			if (insuranceType1) {
				this.insuranceType1 = insuranceType1
			}
			
			const insuranceType2 = insuranceList.find(item => item.insuranceType == 1)
			if (insuranceType2) {
