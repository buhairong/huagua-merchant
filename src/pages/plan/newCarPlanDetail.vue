	...
	methods: {
		...
		async getProductTemplate() {
			...
			if (res.code == 0) {
				...
				this.createData.monthTotal = res.data[0].otherPropertyDto.subscriptionPeriodArray[0]
			}
		...
		prevStep(data) {
			...
			if (data.color) {
				this.createData.colors = data.color
			}
		...
		nextStep(data) {
			...
			// from step 3
			if (data.otherConfigs && this.createData.otherConfigs.length) {
				this.createData.otherConfigs[0].details = data.otherConfigs
			}
			...
			console.log('nextStep', this.createData)
		},
		
		save(data) {
			this.createData.distributionBrokerage = data.distributionBrokerage
			console.log('save', this.createData)
		...
		async savePlan() {
			...
			if (res.code == 0) {
				...
				uni.reLaunch({
					url: `/pages/plan/createSuccess?operator=1&id=${res.data.id}&type=1&imgUrl=${this.productEntity.imageUrl}&brandName=${this.carProductInfo.brandName}&carType=${this.carProductInfo.carType}&carTypeYear=${this.carProductInfo.carTypeYear}&carTypeYearProduct=${this.carProductInfo.carTypeYearProduct}`
				})
			} else {
				uni.showToast({
					title: '保存失败',
		...
		async updatePlan() {
			...
			if (res.code == 0) {
				...
				uni.reLaunch({
					url: `/pages/plan/createSuccess?operator=2&id=${this.createData.id}&type=1&imgUrl=${this.productEntity.imageUrl}&brandName=${this.carProductInfo.brandName}&carType=${this.carProductInfo.carType}&carTypeYear=${this.carProductInfo.carTypeYear}&carTypeYearProduct=${this.carProductInfo.carTypeYearProduct}`
				})
			} else {
				uni.showToast({
					title: '修改失败',
