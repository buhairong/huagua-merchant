<template>
	<view>
		<view class="card">
			...
			<view class="item">
				<view class="item-title">车 &nbsp;&nbsp;&nbsp;&nbsp; 况</view>
				<view class="item-content">全新车</view>
			</view>
			<view class="item">
				<view class="item-title">外观颜色</view>
				<view class="item-content">{{carInfo.carColor || '-'}}</view>
			</view>
			<view class="item">
				<view class="item-title">实表里程</view>
				<view class="item-content">{{carInfo.mileage ? `${carInfo.mileage}公里` : '-'}}</view>
			</view>
			<view class="item">
				<view class="item-title">车 架 号</view>
				<view class="item-content">{{carInfo.carVinFull || '-'}}</view>
			</view>
			<view class="item">
...
export default {
	...
	methods: {
		modifyCarInfo() {
			try {
				uni.setStorageSync('new_car_detail', JSON.stringify(this.carInfo))
				
				uni.navigateTo({
					url: `/pages/car/addNewCar?id=${this.carInfo.id}`
				})
			} catch (e) {
				// error
			}
