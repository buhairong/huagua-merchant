	...
	<view class="card" :class="{'new-car-card': type == 1, 'old-car-card': type == 2}" @click="goDetailPage">
		<view class="title-wrap">
			<view class="title">
				...
				<view class="car-color" v-if="type == 1">{{item.carColor}}</view>
			</view>
			<view class="car-status" v-if="type == 2">
				车辆状态：{{oldCarStatus[item.status].label}}
			</view>
		</view>
		...
		<view class="items-wrap">
			...
			<view class="right">
				<image
					...
					mode="aspectFill"
				/>
				<view class="create-plan-btn" @click.stop="createPlan" v-if="type == 2 && item.status === 1">创建方案</view>
			</view>
		</view>
	</view>
</template>
...
import { oldCarStatus } from '@/constants/index.js'
...
export default {
	...
	methods: {
		...
		goDetailPage() {
			if(this.type == 1) {
				uni.navigateTo({
					url: `/pages/car/newCarDetail?id=${this.item.id}`
				})
			} else {
				uni.navigateTo({
					url: `/pages/car/oldCarDetail?id=${this.item.id}`
				})
			}
		...
		createPlan() {
			uni.navigateTo({
				url: `/pages/plan/oldCarPlanDetail?secondHandCarId=${this.item.id}&brandId=${this.item.carBrandId}&brandName=${this.item.carBrand}&carTypeId=${this.item.carTypeId}&carType=${this.item.carType}&carTypeYearId=${this.item.carTypeYearId}&carTypeYear=${this.item.carTypeYear}&carTypeYearProductId=${this.item.carTypeYearProductId}&carTypeYearProduct=${this.item.carTypeYearProduct}`
			})
		},
	}
}
