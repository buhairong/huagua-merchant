		...
		<view class="info">
			<view>
				...
				<view class="item">
					...
					<view class="item-content">{{item.userName}}{{item.mobile || ''}}</view>
				</view>
				<view class="item">
...
export default {
	...
	mounted() {
		console.log('isOverTimeOfProhibit', this.item.isOverTimeOfProhibit)
	...
	methods: {
		...
		gotoDetail(item) {
			uni.navigateTo({
				url: `/risk/risk/riskOrderDetail?id=${this.item.id}&auditRate=${this.item.auditRate}&orderType=${this.orderType}`
			})
		},
	},
}
